...
const db = require("../Detabase/setup");
...
async function trackStartEventHandler(msgId, channel, player, track, client) {
    try {
        ...
        if (!message) {
            ...
            let embed1 = new EmbedBuilder().setColor(client.embedColor).setDescription(`[${track.title}](${track.uri}) - \`[ ${track.isStream ? '[**◉ LIVE**]' : convertTime(player.current.length)} ]\``).setImage(icon).setFooter({ text: `Requested by ${player.current.requester.tag}`, iconURL: player.current.requester.displayAvatarURL({ dynamic: true }) });

            const but1 = new ButtonBuilder().setCustomId(`${player.guild}pause`).setEmoji(`⏸️`).setStyle(ButtonStyle.Secondary)
            const but2 = new ButtonBuilder().setCustomId(`${player.guild}previous`).setEmoji(`⏮️`).setStyle(ButtonStyle.Secondary)
            const but3 = new ButtonBuilder().setCustomId(`${player.guild}skip`).setEmoji(`⏭️`).setStyle(ButtonStyle.Secondary)
            const but4 = new ButtonBuilder().setCustomId(`${player.guild}voldown`).setEmoji(`🔉`).setStyle(ButtonStyle.Secondary)
            const but5 = new ButtonBuilder().setCustomId(`${player.guild}volup`).setEmoji(`🔊`).setStyle(ButtonStyle.Secondary)
            ...
            let embed2 = new EmbedBuilder().setColor(message.client.embedColor).setDescription(`[${track.title}](${track.uri}) - \`[ ${track.isStream ? '[**◉ LIVE**]' : convertTime(player.current.length)} ]\``).setImage(icon).setFooter({ text: `Requested by ${player.current.requester.tag}`, iconURL: player.current.requester.displayAvatarURL({ dynamic: true }) });
