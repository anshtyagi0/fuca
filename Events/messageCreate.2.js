...
const guildConfigs = require("../Detabase/guildConfig.js")
const users = require("../Detabase/user_xp.js")
...
module.exports = async (client, message) => {
    ...
    if (userData.xp >= reqXP) {
        ...
        const channel = message.guild.channels.cache.get(data.xpLevelUP.channel) || message.channel;
...
function reply(content, channel, message, userData, data, role) {
    ...
    channel.send({ content: content.replace(/{mention}/g, message.author.toString()).replace(/{level}/, userData.level).replace(/{xp}/, userData.xp) });
}
