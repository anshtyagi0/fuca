...
module.exports = async (client, channel) => {
    try {
        ...
        let enrty = await (await channel.guild.fetchAuditLogs()).entries.find(channel => channel.actionType === 'Delete' && channel.targetType === 'Channel')
        let user = channel.guild.members.cache.find(user => user.id === enrty.executorId)
        let f = enrty.changes.find(change => change.key === 'type').old
        let channeltype;
            ...
            if (guilddata.channellog) {
                if (guilddata.logschannel) {
                    let embed = new EmbedBuilder()
                        ...
                        .setDescription(`Channel Deleted <t:${Math.round(channel.createdTimestamp / 1000)}>`)
                        .addFields(
                        ...
                        .setColor("Blurple")
                        .setFooter({ text: `Channel Delete Logs`, iconURL: channel.guild.members.me.displayAvatarURL({ dynamic: true }) })
        ...
        const msg = new webhook.MessageBuilder()
            ...
            .setDescription(`ERROR IN CHANNELDELETE EVENT.\n${err}`)
            .setColor('#000')
