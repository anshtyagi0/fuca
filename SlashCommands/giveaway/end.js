...
module.exports = {
    name: 'giveaway-end',
    description: 'End giveaway and get winners.',
    options: [
        {
            name: 'giveawayid',
    ...
    run: async (client, interaction, prefix) => {
        ...
        try {
            ...
            if (interaction.member.permissions.has("ManageGuild") || interaction.member.permissions.has("Administrator")) {
                ...
                if (data) {
                    if (data.active === true) {
                        ...
                        await interaction.editReply({ content: "Giveaway ended", ephemeral: false })
                        return msg.edit({
                            embeds: [embed],
                            components: [msgbutton]
                        })
                    } else if (data.active === false) {
                        await interaction.editReply({ content: "Giveaway already ended.", ephemeral: false })
                    }
                } else if (!data) {
            ...
            const msg = new webhook.MessageBuilder()
                .setName('ERROR LOGS')
                .setTitle('ERROR')
                .setDescription(`ERROR IN GEND (SLASH) COMMAND.\n${err}`)
                .setColor('#000')
